...
export default (hexo: any) => {
    function resolveHighlight(name: string) {
        if (!name) {
            name = 'github-gist'
        }
        const cssName = name
        ...
        if (fs.existsSync(file)) {
            ...
            css.parse(content)
                .stylesheet!.rules.filter(
                    (rule: css.Rule) =>
                        rule.type === 'rule' &&
                        rule.selectors!.some((selector: string) =>
                            selector.endsWith('.hljs')
                        )
                )
                .flatMap((rule: css.Rule) => rule.declarations)
                .forEach((declaration: any) => {
                    if (
    ...
    function resolvePrism(name: string) {
        if (!name) {
            name = 'default'
        }
